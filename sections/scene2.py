...
def scene2(self: Slide):
    ...
    source3 = Text("Nuestra definición: COMÉTE", font_size=TEXT_XS, color=GREY)
    ...
    self.play(our_definition.animate.scale(0.8).next_to(chart_random1, LEFT, buff=0.3))
    self.play(Write(chart_random1))

    self.next_slide()

    self.play(Transform(chart_random1, chart_consensus))

    self.next_slide()
    self.play(Unwrite(title), Unwrite(our_definition), FadeOut(chart_random1))
